    ...
    cell.addEventListener('click', ()=>{
      ...
      openModal(`
        ...
        <div class="table-wrap"><table class="reg">
          ...
          <tbody>
            ${events.map(e=>`<tr class="row-clickable" data-open-leave="${e.id}"><td>${esc(e.name)}</td><td><span class="tag tag-navy">${LEAVE_TYPES[e.type]||esc(e.type)}</span></td><td>${e.pending?'<span class="tag tag-amber">รออนุมัติ</span>':'<span class="tag tag-green">อนุมัติแล้ว</span>'}</td></tr>`).join('')}
          </tbody>
        </table></div>
