...
function expandLeaveDates(leaves){
  ...
  leaves.forEach(l=>{
    ...
    const sd = toDateOnly(l.startDate);
    let ed = toDateOnly(l.endDate) || sd; // fall back to the start date if end date is missing/malformed
    if(!sd) return;
    let d = new Date(sd+'T00:00:00');
    let end = new Date(ed+'T00:00:00');
    if(isNaN(d)) return;
    if(isNaN(end) || end < d) end = new Date(d.getTime()); // clone, don't alias — otherwise mutating d below also mutates end
    let guard = 0;
