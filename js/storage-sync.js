...
async function syncFromSheet(showToast){
  ...
  try{
    ...
    // one blip isn't worth alarming anyone over, but repeated failures mean
    // this browser is silently stuck on stale data — with ~100 people sharing
    // one sheet, that's exactly what causes "everyone sees something different"
    if(__syncFailCount >= 2) showSyncBanner();
  }
}
...
function showSyncBanner(){
  ...
  document.getElementById('syncBannerText').textContent =
    'เชื่อมต่อ Google Sheet ไม่สำเร็จ — ข้อมูลที่เห็นอาจไม่ใช่ล่าสุด กดปุ่มด้านขวาเพื่อลองซิงค์ใหม่';
...
function updateSyncStatus(kind){
  ...
  else if(kind==='bad'){ dot.className='sync-dot sync-bad'; text.textContent='เชื่อมต่อล้มเหลว (คลิกเพื่อลองใหม่)'; }
}
