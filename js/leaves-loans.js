...
function viewLeaves(){
  ...
  <div class="pill-nav">
    <button data-leaveview="list" class="active">รายการ</button>
    <button data-leaveview="calendar">ปฏิทิน</button>
  </div>
  <div id="leavesViewBody">
    <div class="panel">
      <div class="searchbar">
...
function bindLeavesPillNav(){
  const pills = document.querySelectorAll('[data-leaveview]');
  pills.forEach(p=>p.addEventListener('click', ()=>{
    pills.forEach(x=>x.classList.remove('active'));
    p.classList.add('active');
    renderLeavesBody(p.dataset.leaveview);
  }));
}

function renderLeavesBody(mode){
  mode = mode || 'list';
  const body = document.getElementById('leavesViewBody');
  if(!body) return;
  if(mode==='calendar'){
    body.innerHTML = `<div class="panel">${leaveCalendarHtml(adminLeaveCalState, state.leaves)}</div>`;
    bindLeaveCalendarEvents(body, adminLeaveCalState, state.leaves, ()=>renderLeavesBody('calendar'));
  } else {
    body.innerHTML = `
    <div class="panel">
      <div class="searchbar">
