/* ===== leaves-loans.js ===== */
/* ================= LEAVES ================= */
/* Reminder: who's on leave tomorrow, so whoever's reading the Leaves page
   can plan staffing without having to scan the whole calendar. */
function tomorrowLeaves(){
  const t = new Date(); t.setDate(t.getDate()+1);
  const tIso = `${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,'0')}-${String(t.getDate()).padStart(2,'0')}`;
  const td = parseYMD(tIso);
  return state.leaves.filter(l=>{
    if((l.approvalStatus||'approved')==='rejected') return false;
...
function tomorrowLeaveReminderHtml(){
  const list = tomorrowLeaves();
  if(!list.length) return '';
  const t = new Date(); t.setDate(t.getDate()+1);
  const tIso = `${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,'0')}-${String(t.getDate()).padStart(2,'0')}`;
  return `
  <div class="panel leave-reminder">
    <div class="leave-reminder-title">🔔 พรุ่งนี้ (${buddhistDate(tIso)}) มีผู้ลา ${list.length} คน</div>
    <div class="leave-reminder-list">
      ${list.map(l=>`<span class="leave-reminder-chip">${avatarHtml(employeeById(l.employeeId),22)} ${esc(l.employeeName)} ${leaveTypeTag(l.leaveType)}${(l.approvalStatus||'approved')==='pending'?' <span class="tag tag-amber">รออนุมัติ</span>':''}</span>`).join('')}
    </div>
  </div>`;
}
...
function leaveRows(){
  ...
  return [...state.leaves]
    ...
    .sort((a,b)=> new Date(b.updatedAt||b.submitDate||0) - new Date(a.updatedAt||a.submitDate||0));
}
...
function renderLeaveTable(){
  ...
  tbody.innerHTML = rows.length ? rows.map((l,i)=>`
    <tr class="row-clickable" data-leave-id="${l.id}">
      ...
      <td>${avatarHtml(employeeById(l.employeeId),26)} <span style="margin-left:7px;">${esc(l.employeeName)}</span></td>
