...
function blankOr(v){
  if(v===undefined || v===null || v==='') return '<span style="display:inline-block; min-width:60px; border-bottom:1px dotted var(--ink-faint);">&nbsp;</span>';
  return esc(v);
}
...
function leaveStatsTableHtml(empId){
  const s = computeEmployeeSummary(empId);
  return `
  <table class="reg" style="margin-top:6px;">
    <thead><tr><th>ประเภทวันลา</th><th class="num">สิทธิ์รวม</th><th class="num">ใช้ไปแล้ว</th><th class="num">คงเหลือ</th></tr></thead>
...
function sickPersonalFormPrintHtml(rec, emp, submit){
  ...
  return `
  <div style="max-width:760px; margin:0 auto; font-size:14px; line-height:2;">
    <div style="text-align:center; font-size:18px; font-weight:700; margin-bottom:6px;">ใบลาป่วย ลาคลอด และลากิจ</div>
    <div style="text-align:right;">เขียนที่ ${esc(ORG_FULL_NAME)}</div>
    <div style="text-align:right;">วันที่ ${blankOr(submit.day)} เดือน ${blankOr(submit.month)} พ.ศ. ${blankOr(submit.year)}</div>
    <div>เรื่อง&emsp;ขออนุญาตลา${isSick?'ป่วย':isPersonal?'กิจ':''}</div>
    <div>เรียน&emsp;หัวหน้าสำนักงาน</div>
    <div style="text-indent:40px;">ข้าพเจ้า ${blankOr(emp.name)} ตำแหน่ง ${blankOr(emp.position)}</div>
    <div>สังกัด (สถานี/สำนักงาน) ${blankOr(emp.department)} ${esc(ORG_FULL_NAME)}</div>
    <div style="text-indent:40px;">ขอลา ${checkbox(isSick)}ป่วย เนื่องจาก ${blankOr(rec.reason)}&emsp;&emsp;${checkbox(isPersonal)}ลากิจส่วนตัว เนื่องจาก ${!isSick?blankOr(rec.reason):blankOr('')}</div>
    <div style="margin-right:40px;">${checkbox(false)}คลอดบุตร&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;${checkbox(false)}อื่นๆ ${blankOr('')}</div>
    <div>ตั้งแต่วันที่ ${blankOr(buddhistDate(rec.startDate))} ถึงวันที่ ${blankOr(buddhistDate(rec.endDate))} มีกำหนด ${blankOr(rec.days)} วัน</div>
    <div>ข้าพเจ้าได้ลา ${checkbox(isSick)}ป่วย &emsp;${checkbox(isPersonal)}กิจส่วนตัว &emsp;${checkbox(false)}คลอดบุตร &emsp;${checkbox(false)}อื่นๆ</div>
    <div>ครั้งสุดท้ายตั้งแต่วันที่ ${prev?blankOr(buddhistDate(prev.startDate)):blankOr('')} ถึงวันที่ ${prev?blankOr(buddhistDate(prev.endDate)):blankOr('')} มีกำหนด ${prev?blankOr(prev.days):blankOr('')} วัน</div>
    <div>ในระหว่างลาจะติดต่อข้าพเจ้าได้ที่บ้านเลขที่ ${blankOr(rec.contactAddress)} ตรอก/ซอย ${blankOr(rec.contactSoi)} ตำบล/แขวง ${blankOr(rec.contactTambon)}</div>
    <div>อำเภอ/เขต ${blankOr(rec.contactAmphoe)} จังหวัด ${blankOr(rec.contactProvince)}</div>
    <div>โทรศัพท์ ${blankOr(rec.contactPhone)}</div>
    <div style="text-align:right; margin-top:24px;">ขอแสดงความนับถือ</div>
    <div style="text-align:right; margin-top:36px;">.......................................................</div>
    <div style="text-align:right;">( ${esc(emp.name||'')} )</div>

    <div class="doc-section-title" style="margin-top:22px;">สถิติการลาในปีนี้</div>
    ${leaveStatsTableHtml(rec.employeeId)}

    <div style="margin-top:26px; display:flex; justify-content:space-between; gap:30px;">
      <div style="flex:1;">
        <div>ความเห็นผู้บังคับบัญชา</div>
        <div style="margin-top:24px; border-bottom:1px dotted var(--ink-faint);">&nbsp;</div>
        <div style="margin-top:20px; border-bottom:1px dotted var(--ink-faint);">&nbsp;</div>
        <div style="margin-top:20px;">ลงชื่อ .......................................... วันที่ ...../...../.....</div>
      </div>
      <div style="flex:1;">
        <div>คำสั่ง&emsp;${checkbox(false)} อนุญาต&emsp;&emsp;${checkbox(false)} ไม่อนุญาต</div>
        <div style="margin-top:44px;">ลงชื่อ ..........................................</div>
        <div>( .......................................... )</div>
...
function vacationFormPrintHtml(rec, emp, submit){
  ...
  return `
  <div style="max-width:760px; margin:0 auto; font-size:14px; line-height:2;">
    <div style="text-align:center; font-size:18px; font-weight:700; margin-bottom:6px;">แบบใบลาพักผ่อน</div>
    <div style="text-align:right;">เขียนที่ ${esc(ORG_FULL_NAME)}</div>
    <div style="text-align:right;">วันที่ ${blankOr(submit.day)} เดือน ${blankOr(submit.month)} พ.ศ. ${blankOr(submit.year)}</div>
    <div>เรื่อง&emsp;ขออนุญาตลาพักผ่อน</div>
    <div>เรียน&emsp;หัวหน้าสำนักงานมูลนิธิฯ</div>
    <div style="text-indent:40px;">ข้าพเจ้า ${blankOr(emp.name)} ตำแหน่ง ${blankOr(emp.position)}</div>
    <div>มีวันลาพักผ่อนสะสม ${blankOr(vac.carry)} วันทำการ มีสิทธิพักผ่อนประจำปีนี้อีก ${blankOr(vac.right)} วันทำการ รวมเป็น ${blankOr(vac.total)} วันทำการ</div>
    <div>ขอลาพักผ่อนตั้งแต่วันที่ ${blankOr(buddhistDate(rec.startDate))} ถึง ${blankOr(buddhistDate(rec.endDate))} มีกำหนด ${blankOr(rec.days)} วันทำการ</div>
    <div>ในระหว่างลาจะติดต่อข้าพเจ้าได้ที่บ้านเลขที่ ${blankOr(rec.contactAddress)} ตรอก/ซอย ${blankOr(rec.contactSoi)} ตำบล/แขวง ${blankOr(rec.contactTambon)}</div>
    <div>อำเภอ/เขต ${blankOr(rec.contactAmphoe)} จังหวัด ${blankOr(rec.contactProvince)}</div>
    <div>โทรศัพท์ ${blankOr(rec.contactPhone)}</div>
    <div style="text-align:right; margin-top:24px;">ขอแสดงความนับถือ</div>
    ...
    <div style="margin-top:30px;">
      <div>คำสั่ง&emsp;${checkbox(false)} อนุญาต&emsp;&emsp;${checkbox(false)} ไม่อนุญาต</div>
      <div style="text-align:right; margin-top:34px;">ลงชื่อ ..........................................</div>
      <div style="text-align:right;">( .......................................... )</div>
      <div style="text-align:right; margin-top:8px;">วันที่ ...../...../.....</div>
    </div>
  </div>`;
}
