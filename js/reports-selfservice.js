...
function viewMyLeaves(){
  ...
  <div class="pill-nav">
    <button data-myleaveview="list" class="active">รายการ</button>
    <button data-myleaveview="calendar">ปฏิทิน</button>
  </div>
  <div id="myLeavesViewBody">
    <div class="panel">
...
function renderMyLeavesBody(mode){
  mode = mode || 'list';
  const empId = currentEmployeeId();
  const rows = state.leaves.filter(l=>l.employeeId===empId).sort((a,b)=> new Date(b.startDate||0)-new Date(a.startDate||0));
  const body = document.getElementById('myLeavesViewBody');
  if(!body) return;
  if(mode==='calendar'){
    body.innerHTML = `<div class="panel">${leaveCalendarHtml(selfLeaveCalState, rows)}</div>`;
    bindLeaveCalendarEvents(body, selfLeaveCalState, rows, ()=>renderMyLeavesBody('calendar'));
  } else {
    body.innerHTML = `
    <div class="panel">
      <div class="table-wrap"><table class="reg" id="myLeaveTable">
    ...
    bindSelfRecordActions(body, { leaves: (id)=>openLeaveModal(id) });
  }
}
