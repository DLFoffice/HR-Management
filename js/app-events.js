...
function bindViewEvents(v){
  if(v==='dashboard'){
    ...
    document.getElementById('btnPrintOrgChart')?.addEventListener('click', ()=> printDocument(orgChartBodyHtml(), {title: 'โครงสร้างองค์กร'}));
    ...
    bindLeaveCalendarEvents(document.getElementById('leaveCalendarPanel'), adminLeaveCalState, state.leaves, refreshAdminLeaveCalendar);
    ...
    document.getElementById('btnAddLeave')?.addEventListener('click', ()=>openLeaveModal(null));
    const empIdForCal = currentEmployeeId();
    const rowsForCal = state.leaves.filter(l=>l.employeeId===empIdForCal);
    bindLeaveCalendarEvents(document.getElementById('myLeaveCalendarPanel'), selfLeaveCalState, rowsForCal, refreshSelfLeaveCalendar);
    ...
    document.querySelectorAll('#myLeaveTable [data-print-leave]').forEach(b=>b.addEventListener('click', (e)=>{
      e.stopPropagation();
    ...
    document.getElementById('btnSyncFromDiag')?.addEventListener('click', ()=>syncFromSheet(true));
    document.getElementById('btnSaveFirebaseConfig')?.addEventListener('click', async ()=>{
      ...
      if(!text){ toast('กรุณาวางโค้ด firebaseConfig ก่อน'); return; }
      try{
        const cfg = parseFirebaseConfigText(text);
        state.settings.firebaseConfig = cfg;
        await saveSettings();
        firebaseApp = null; firebaseDb = null; // force re-init with new config
        initFirebaseIfConfigured();
        toast(firebaseReady() ? 'เชื่อมต่อ Firebase สำเร็จ' : 'บันทึกค่าแล้ว แต่เชื่อมต่อไม่สำเร็จ ตรวจสอบ config อีกครั้ง');
        renderView();
      }catch(e){
        toast('ตั้งค่า Firebase ไม่สำเร็จ: '+e.message);
      }
    });
    document.getElementById('btnBulkPushLogins')?.addEventListener('click', async ()=>{
      if(!firebaseReady()){ toast('กรุณาเชื่อมต่อ Firebase ก่อน'); return; }
      toast('กำลังซิงค์เลขบัตรไปยัง Firebase...');
      const result = await firestoreBulkPushAllLogins();
      toast('ซิงค์ไปยัง Firebase สำเร็จ '+result.count+' คน');
    });
    document.getElementById('btnImportIdCardExcel')?.addEventListener('click', async ()=>{
      const fileInput = document.getElementById('idCardExcelFile');
      const file = fileInput.files[0];
      const logEl = document.getElementById('idCardImportLog');
      if(!file){ toast('กรุณาเลือกไฟล์ Excel ก่อน'); return; }
      if(!firebaseReady()){ toast('กรุณาเชื่อมต่อ Firebase ก่อน'); return; }
      logEl.innerHTML = '<div class="muted" style="font-size:13px;">กำลังอ่านไฟล์...</div>';
      try{
        const buf = await file.arrayBuffer();
        const wb = XLSX.read(buf, {type:'array'});
        const sheet = wb.Sheets[wb.SheetNames[0]];
        const rows = XLSX.utils.sheet_to_json(sheet, {defval:''});
        if(rows.length === 0){ logEl.innerHTML = '<div style="color:var(--red);">ไฟล์นี้ไม่มีข้อมูล</div>'; return; }

        const results = [];
        for(const row of rows){
          const idCard = String(row.idCard||'').trim();
          const code = String(row.code||'').trim();
          const nameInFile = String(row.name||'').trim();
          if(!idCard){ results.push({name:nameInFile||'(ไม่ทราบชื่อ)', status:'skip', msg:'ไม่มีเลขบัตรในไฟล์แถวนี้'}); continue; }

          const emp = state.employees.find(e => String(e.code||'').trim()===code) 
                    || state.employees.find(e => normalizeNameForMatch(e.name)===normalizeNameForMatch(nameInFile));
          if(!emp){ results.push({name:nameInFile||code, status:'nomatch', msg:'ไม่พบพนักงานที่ตรงกันในระบบ (รหัส '+code+')'}); continue; }

          // update locally + push to Google Sheet (best-effort) + push to Firebase (the important part)
          emp.idCard = idCard;
          try{
            await firebaseDb.collection('employeeLogins').doc(idCard).set({ employeeId: emp.id, name: emp.name });
            results.push({name:emp.name, status:'ok', msg:'บันทึกเข้า Firebase สำเร็จ'});
          }catch(err){
            results.push({name:emp.name, status:'error', msg:'Firebase error: '+err.message});
          }
        }
        await saveCache();
        if(gasReady()){
          // best-effort: also push idCard updates back to the Google Sheet, doesn't block the Firebase result
          state.employees.filter(e=>e.idCard).forEach(e=>{ gasPost({action:'update', sheet:'employees', data:e}).catch(()=>{}); });
        }

        const okCount = results.filter(r=>r.status==='ok').length;
        const errCount = results.filter(r=>r.status==='error').length;
        const nomatchCount = results.filter(r=>r.status==='nomatch').length;
        const logHtml = `
          <div style="display:flex; gap:16px; margin-bottom:10px; font-size:13.5px;">
            <span style="color:var(--green); font-weight:600;">✓ สำเร็จ ${okCount}</span>
            <span style="color:var(--red); font-weight:600;">✗ ผิดพลาด ${errCount}</span>
            <span style="color:var(--amber); font-weight:600;">? ไม่พบคนตรงกัน ${nomatchCount}</span>
          </div>
          <div class="table-wrap" style="max-height:320px; overflow-y:auto;">
            <table class="reg">
              <thead><tr><th>ชื่อ/รหัส</th><th>สถานะ</th><th>รายละเอียด</th></tr></thead>
              <tbody>
                ${results.map(r=>`<tr><td>${esc(r.name)}</td><td>${r.status==='ok'?'<span class="tag tag-green">สำเร็จ</span>':r.status==='error'?'<span class="tag tag-red">ผิดพลาด</span>':'<span class="tag tag-amber">ไม่พบ</span>'}</td><td style="font-size:12.5px;">${esc(r.msg)}</td></tr>`).join('')}
              </tbody>
            </table>
          </div>`;
        // renderView() rebuilds the whole page (to refresh the employee counts elsewhere
        // on this page), which would wipe out the log div — so re-render FIRST, then
        // re-select the freshly-created log element and write the results into it last.
        renderView();
        const freshLogEl = document.getElementById('idCardImportLog');
        if(freshLogEl) freshLogEl.innerHTML = logHtml;
      }catch(e){
        logEl.innerHTML = `<div style="color:var(--red); font-size:13.5px;">อ่านไฟล์หรือประมวลผลไม่สำเร็จ: ${esc(e.message)}</div>`;
      }
    });
...
async function init(){
  ...
  renderBrand();
  initFirebaseIfConfigured();
  document.getElementById('sidebarFoot')?.addEventListener('click', ()=>syncFromSheet(true));
  document.getElementById('syncBannerRetry')?.addEventListener('click', ()=>syncFromSheet(true));
...
/* Periodically re-syncs with Google Sheet in the background so different
   people using the system at the same time don't drift too far out of sync
   with each other (e.g. an admin's approval showing up for a logged-in
   employee without them needing to log out and back in). Runs quietly —
   no toast, no view refresh unless the person is actually looking at the app. */
/* Periodically re-syncs with Google Sheet in the background so different
   people using the system at the same time don't drift too far out of sync
   with each other (e.g. an admin's approval showing up for a logged-in
   employee without them needing to log out and back in). Runs quietly —
   no toast, no view refresh unless the person is actually looking at the app.
   Uses a self-rescheduling timer (not a fixed setInterval) so a failed sync
   retries sooner instead of silently waiting out the full interval, and adds
   random jitter so ~100 clients don't all hit the Apps Script endpoint at
   the exact same moment and trip its concurrent-execution limits. */
const AUTO_SYNC_BASE_MS = 60000;
const AUTO_SYNC_RETRY_MS = 12000;
function startAutoSync(){
  const tick = async ()=>{
    let delay = AUTO_SYNC_BASE_MS + Math.floor(Math.random()*8000);
    if(gasReady() && state.currentUser && !document.hidden){
      const before = __syncFailCount;
      await syncFromSheet(false);
      if(__syncFailCount > before){
        // back off a bit faster than the normal cadence, but don't hammer
        // the endpoint either — cap the retry delay well under the base interval
        delay = Math.min(AUTO_SYNC_RETRY_MS * Math.min(__syncFailCount, 4), AUTO_SYNC_BASE_MS);
      }
    }
    setTimeout(tick, delay);
  };
  setTimeout(tick, AUTO_SYNC_BASE_MS + Math.floor(Math.random()*8000));
}
...
function bindLoginForm(){
  ...
  const tryLogin = async ()=>{
    ...
    let ok = await attemptLogin(uEl.value, pEl.value);
    if(!ok && gasReady() && !firebaseReady()){
      // data might be stale on this device — re-sync once and retry before giving up
      // (only relevant when logins still rely on the legacy Google-Sheet fallback)
      setLoginBusy(true, 'กำลังตรวจสอบข้อมูลล่าสุด...');
      await syncFromSheet(false);
      setLoginBusy(false);
      ok = await attemptLogin(uEl.value, pEl.value);
    }
    if(ok){
      afterLogin();
      updateSyncStatus(gasReady() ? (state.lastSync? 'ok':'none') : 'none');
