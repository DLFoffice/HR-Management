...
function toDateOnly(v){
  ...
  if(m) return `${m[1]}-${m[2]}-${m[3]}`;
  // JS Date.toString() output (e.g. a Google Apps Script backend that stringified a
  // Date object before saving it): "Mon Sep 14 2026 00:00:00 GMT+0700 (...)". Read the
  // month name + day + year directly out of the text — never through `new Date(str)` —
  // so the result doesn't shift by a day depending on the browser's local timezone.
