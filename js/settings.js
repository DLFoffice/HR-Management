...
function viewSettings(){
  ...
  <div class="panel">
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:12px; flex-wrap:wrap;">
      <div style="font-size:13.5px; color:var(--ink-soft);">
        ซิงค์ล่าสุด: <b>${state.lastSync ? fmtDateTime(state.lastSync) : 'ยังไม่เคยซิงค์'}</b> — ถ้าเพิ่งแก้ Google Sheet มา ให้กดปุ่มนี้ก่อนตรวจสอบเสมอ
      </div>
      <button class="btn btn-sm btn-primary" id="btnSyncFromDiag">ซิงค์ข้อมูลตอนนี้</button>
    </div>
    <div style="font-size:13.5px; color:var(--ink-soft); margin-bottom:12px;">
    ...
    <div style="margin-top:16px; padding-top:14px; border-top:1px solid var(--line);">
      <div class="muted" style="font-size:12.5px; margin-bottom:8px;">
        <b>ค่า idCard ดิบที่ระบบอ่านได้ (10 คนแรก)</b> — ถ้าคุณเพิ่งใส่เลขบัตรให้พนักงานคนไหนไปแล้ว ต้องเห็นเลขนั้นในลิสต์นี้ ถ้าทุกช่องว่างเปล่าหมด แปลว่าระบบยังไม่ได้รับค่าจริงจากชีตเลย (ปัญหาน่าจะอยู่ที่การซิงค์ หรือแถวที่พิมพ์ไม่ตรงกับพนักงานที่ตั้งใจ)
      </div>
      <div class="table-wrap" style="max-height:220px; overflow-y:auto;">
        <table class="reg">
          <thead><tr><th class="rownum">#</th><th>ชื่อ-นามสกุล</th><th>ค่า idCard ดิบ (มีเครื่องหมาย " ครอบไว้)</th></tr></thead>
          <tbody>
            ${state.employees.slice(0,10).map((e,i)=>`<tr><td class="rownum">${i+1}</td><td>${esc(e.name)}</td><td style="font-family:monospace;">"${esc(e.idCard)}"</td></tr>`).join('')}
          </tbody>
        </table>
      </div>
    </div>
  ...
  <div class="panel">
    <h2>8. เชื่อมต่อ Firebase สำหรับล็อกอินพนักงาน (แนะนำ — แก้ปัญหาซิงค์ Google Sheet ได้เด็ดขาด)</h2>
    <div style="font-size:13.5px; color:var(--ink-soft); margin-bottom:12px;">
      ระบบล็อกอินเดิมอ่านเลขบัตรประชาชนจาก Google Sheet ซึ่งอาจมีปัญหาการซิงค์/ชื่อคอลัมน์/แถวไม่ตรงได้ง่าย วิธีนี้จะแยกระบบล็อกอินออกมาเก็บใน <b>Firebase (Firestore)</b> โดยเฉพาะ — เวลากรอกเลขบัตรที่หน้า "ข้อมูลพนักงาน" เหมือนเดิมทุกอย่าง แต่ระบบจะบันทึกข้อมูลล็อกอินลง Firebase ไปพร้อมกันด้วย ทำให้การล็อกอินไม่ต้องพึ่งการซิงค์ Google Sheet อีกต่อไป
    </div>
    <div class="status-line" style="margin-bottom:12px;">
      <span class="sync-dot ${firebaseReady()?'sync-ok':'sync-none'}"></span>
      <span>${firebaseReady() ? 'เชื่อมต่อ Firebase แล้ว' : (firebaseConfigured() ? 'ตั้งค่าไว้แล้วแต่เชื่อมต่อไม่สำเร็จ ลองบันทึกใหม่' : 'ยังไม่ได้เชื่อมต่อ Firebase (ระบบจะใช้วิธีเดิมอ่านจาก Google Sheet ไปก่อน)')}</span>
    </div>
    <div class="field full">
      <label>วางโค้ด firebaseConfig ทั้งบล็อกจาก Firebase Console ที่นี่</label>
      <textarea id="firebaseConfigInput" style="min-height:110px; font-family:monospace; font-size:12.5px;" placeholder='const firebaseConfig = {
  apiKey: "AIza...",
  ...
  messagingSenderId: "...",
  appId: "..."
};'>${state.settings.firebaseConfig ? JSON.stringify(state.settings.firebaseConfig, null, 2) : ''}</textarea>
    </div>
    <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:8px;">
      <button class="btn btn-primary btn-sm" id="btnSaveFirebaseConfig">บันทึกการตั้งค่า Firebase</button>
      <button class="btn btn-gold btn-sm" id="btnBulkPushLogins" ${firebaseReady()?'':'disabled'}>ซิงค์เลขบัตรทั้งหมดจาก Google Sheet ไปยัง Firebase ครั้งเดียว</button>
    </div>
    <div class="steps" style="margin-top:16px;">
      <ol>
        <li>ไปที่ <code class="inline">console.firebase.google.com</code> → สร้างโปรเจกต์ใหม่ (หรือใช้โปรเจกต์เดิม)</li>
        <li>เมนูซ้าย <code class="inline">Build → Firestore Database</code> → กด <code class="inline">Create database</code> → เลือกโหมด <code class="inline">Test mode</code> เพื่อเริ่มต้นง่ายๆ ก่อน</li>
        <li>ไปที่ <code class="inline">Project settings</code> (รูปเฟือง) → เลื่อนลงหา <code class="inline">Your apps</code> → กดไอคอน <code class="inline">&lt;/&gt;</code> เพื่อสร้างเว็บแอป → ตั้งชื่ออะไรก็ได้ → กด Register app</li>
        <li>คัดลอกโค้ด <code class="inline">const firebaseConfig = {...}</code> ที่ขึ้นมาทั้งหมด มาวางในช่องด้านบน แล้วกด "บันทึกการตั้งค่า Firebase"</li>
        <li>กด "ซิงค์เลขบัตรทั้งหมด...ไปยัง Firebase" เพื่อย้ายเลขบัตรที่มีอยู่แล้วใน Google Sheet เข้า Firebase ในคราวเดียว (ทำครั้งเดียวพอ)</li>
      </ol>
    </div>
    <div class="muted" style="font-size:12px; margin-top:10px;">
      ⚠️ โหมด "Test mode" ของ Firestore จะหมดอายุใน 30 วันและเปิดให้อ่าน/เขียนได้อย่างเปิดกว้าง เหมาะสำหรับทดลองใช้งานก่อน ถ้าจะใช้งานจริงระยะยาวควรตั้งค่า Firestore Security Rules ให้เหมาะสมภายหลัง
    </div>
  </div>

  <div class="panel">
    <h2>9. นำเข้าเลขบัตรประชาชนจากไฟล์ Excel (เข้า Firebase โดยตรง)</h2>
    <div style="font-size:13.5px; color:var(--ink-soft); margin-bottom:12px;">
      อัปโหลดไฟล์ Excel ที่มีคอลัมน์ <code class="inline">idCard</code>, <code class="inline">code</code>, <code class="inline">name</code> ระบบจะจับคู่กับพนักงานในระบบด้วย "รหัสพนักงาน (code)" แล้วบันทึกเลขบัตรเข้า Firebase ให้ทุกคนในคราวเดียว พร้อม<b>แสดงผลลัพธ์จริงทีละคน</b> (สำเร็จ/ไม่สำเร็จ พร้อมข้อความ error ถ้ามี) เพื่อให้เห็นสาเหตุชัดเจนถ้ามีปัญหา
    </div>
    <div style="display:flex; align-items:center; gap:10px; margin-bottom:12px;">
      <input type="file" id="idCardExcelFile" accept=".xlsx,.xls">
      <button class="btn btn-primary btn-sm" id="btnImportIdCardExcel" ${firebaseReady()?'':'disabled'}>นำเข้าและบันทึกเข้า Firebase</button>
    </div>
    ${!firebaseReady() ? '<div class="muted" style="font-size:12px; color:var(--red);">ต้องเชื่อมต่อ Firebase ที่ข้อ 8 ให้สำเร็จก่อน (สถานะต้องขึ้น "เชื่อมต่อ Firebase แล้ว") ปุ่มนี้จึงจะกดได้</div>' : ''}
    <div id="idCardImportLog" style="margin-top:12px;"></div>
  </div>
  `;
}
