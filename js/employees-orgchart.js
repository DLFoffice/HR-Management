...
function groupEmployeesByDept(rows){
  ...
  return Object.keys(groups)
    ...
    .map(d=>({ dept: d, list: groups[d].sort((a,b)=> yearsOfServiceNum(b.hireDate)-yearsOfServiceNum(a.hireDate)) }));
}
    ...
    <div class="panel dept-box">
      ...
      <div class="table-wrap"><table class="reg">
        <thead><tr>
          ...
          <th class="num">อายุ</th><th class="num">อายุงาน</th><th class="num">วันเกษียณ</th><th>จัดการ</th>
        </tr></thead>
        <tbody>
          ${g.list.map((e,i)=>`
          <tr class="row-clickable" data-emp-id="${e.id}">
            ...
            <td class="num">${tenureBadgeHtml(e.hireDate)}</td>
